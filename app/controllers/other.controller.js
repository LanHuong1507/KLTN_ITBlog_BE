...
class OtherController {
  ...
  async topInteract(req, res) {
    try {
      ...
      res.status(200).json({
        ...
        articles: await sequelize.query(query, {
          type: sequelize.QueryTypes.SELECT,
        }),
      });
    } catch (error) {
  ...
  async topTrending(req, res) {
    ...
    `,
      ...
      if (Array.isArray(topViews)) {
        topViews.forEach(
          (article) => (uniqueArticles[article.article_id] = article)
        );
      }
      ...
      const articles = Object.values(uniqueArticles).sort(
      ...
      res.status(200).json({
        message:
          "Danh sách bài viết Top Trending theo lượt xem, lượt thích và bình luận",
        articles,
      });
    } catch (error) {
      res
        .status(500)
        .json({ message: "Lỗi khi lấy bài viết top trending", error });
    }
  }
  ...
  async topCategories(req, res) {
    try {
      ...
      res.status(200).json({
        ...
        categories: await sequelize.query(query, {
          type: sequelize.QueryTypes.SELECT,
        }),
      });
    } catch (error) {
  ...
  async topRelated(req, res) {
    try {
      ...
      const tagsConditions = tags
        .map((tag) => `tags LIKE '%${tag}%'`)
        .join(" OR "); // e.g., "tags LIKE '%tag1%' OR tags LIKE '%tag2%'"
      ...
      res
        .status(500)
        .json({ message: "Có lỗi xảy ra khi lấy bài viết liên quan" });
    }
  }
  ...
  async topPopularToday(req, res) {
    try {
      ...
      if (results.length === 0) {
        return res
          .status(404)
          .json({ message: "Không tìm thấy bài viết phù hợp." });
      }
      ...
      return res
        .status(500)
        .json({ message: "Có lỗi xảy ra khi lấy bài viết.", error });
    }
  }
  ...
  async getArticlesFollowing(req, res) {
    try {
      ...
      const articlesQuery = `
            SELECT  
            a.article_id, a.slug, a.title, a.content, a.image_url, a.createdAt, 
            u.username, u.avatar_url, u.fullname, 
