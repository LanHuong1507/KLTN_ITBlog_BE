...
class AuthController {
  ...
  async login(req, res) {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(400)
          .json({ message: "Email hoặc tên người dùng không đúng" });
      }
      ...
      return res
        .status(200)
        .json({ message: "Đăng nhập thành công", token, refreshToken });
    } catch (error) {
  ...
  async refreshToken(req, res) {
    ...
    if (!refreshToken) {
      return res
        .status(400)
        .json({ message: "Vui lòng cung cấp refresh token" });
    }
    ...
    try {
      ...
      if (!user)
        return res
          .status(400)
          .json({ message: "Tài khoản không tồn tại", error });

      if (user.role == "blocked")
        return res
          .status(400)
          .json({ message: "Tài khoản bị cấm khỏi hệ thống", error });
      ...
      return res
        .status(400)
        .json({ message: "Refresh token không hợp lệ", error });
    }
  }
  ...
  async passwordReset(req, res) {
    ...
    try {
      ...
      return res
        .status(200)
        .json({ message: "Đã gửi yêu cầu đặt lại mật khẩu qua email." });
    } catch (error) {
      console.error("Lỗi yêu cầu đặt lại mật khẩu:", error);
      return res
        .status(500)
        .json({ message: "Lỗi yêu cầu đặt lại mật khẩu", error });
    }
  }
  ...
  async resetPassword(req, res) {
    ...
    if (!token) {
      return res
        .status(400)
        .json({ message: "Yêu cầu đổi mật khẩu không hợp lệ" });
    }

    if (!newPassword) {
      return res
        .status(400)
        .json({ message: "Vui lòng nhập mật khẩu cần đổi" });
    }
    ...
    if (!user) {
      return res
        .status(400)
        .json({
          message: "Yêu cầu đổi mật khẩu không hợp lệ hoặc đã hết hạn.",
        });
    }
